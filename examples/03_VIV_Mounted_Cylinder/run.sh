...
# Test Case: Vortex-Induced Vibration with Fluid-Structure Interaction
#
# Usage:
...
do_preprocess() {
    ...
    if [ -f "grid.dat" ]; then
        ...
        if [ -L "grid.dat" ]; then
            echo "       (symlink to $(readlink grid.dat))"
        fi
        # Get file size
    ...
    if [ -f "ibmdata00" ]; then
        ...
        echo "         This file contains the cylinder surface mesh for IBM."
        errors=$((errors + 1))
    fi
    ...
    if [ -f "control.dat" ]; then
        ...
        if [ "${USE_XML}" -ne 1 ] || [ ! -f "control.xml" ]; then
            echo "  [ERROR] No configuration file found!"
            errors=$((errors + 1))
        fi
    fi

    print_step "Validating IBM setup..."

    # Check for required IBM parameters in XML
    if [ -f "control.xml" ]; then
        if grep -q "immersed_boundary.*enabled=\"1\"" control.xml; then
            echo "  [OK] Immersed Boundary Method enabled"
        else
            echo "  [WARNING] IBM may not be enabled in control.xml"
        fi
        if grep -q "fsi.*enabled=\"1\"" control.xml; then
            echo "  [OK] Fluid-Structure Interaction enabled"
        else
            echo "  [WARNING] FSI may not be enabled in control.xml"
        fi
    fi
    ...
    if [ ${errors} -eq 0 ]; then
        ...
        echo ""
        echo "  Test case: VIV Mounted Cylinder"
        echo "  Physics:   Re=150, single-phase, laminar"
        echo "  FSI:       1-DOF (Y-direction), mass ratio=0.25"
        echo "  Expected:  Lock-in oscillations due to vortex shedding"
    else
...
do_simulate() {
    ...
    if [ ! -f "grid.dat" ]; then
        echo "ERROR: grid.dat not found!"
        echo "       Run './run.sh preprocess' to check all requirements."
        exit 1
    fi
    if [ ! -f "ibmdata00" ]; then
    ...
    echo "  Re:         150 (laminar vortex shedding)"
    echo "  U_red:      6 (reduced velocity - resonance region)"
    echo "  Mass ratio: 0.25"
    echo "  Damping:    0.0 (undamped)"
    ...
    if [ -f "Force_Coeff_SI00_00" ]; then
        echo "  Force coefficients: Force_Coeff_SI00_00"
    fi
}
...
do_postprocess() {
    ...
    # Note: IBM case, no levelset, binary grid format
    POST_CMD="${BUILD_DIR}/Source/data -tis ${TIS} -tie ${TIE} -ts ${TS} -vtk 1 -xyz 0 -binary 1 -levelset 0"
    ...
    # Analyze FSI data if available
    if [ -f "FSI_position00" ]; then
        print_step "FSI Analysis"
        echo "  Cylinder displacement history: FSI_position00"
        echo ""
        echo "  To visualize:"
        echo "    - Plot column 2 (Y displacement) vs column 1 (time)"
        echo "    - Expected: sinusoidal oscillation after transient"
        echo "    - Look for lock-in: constant amplitude oscillation"
    fi
}
...
do_clean() {
    ...
    rm -f su0_*.dat su1_*.dat su2_*.dat sp_*.dat sp2_*.dat
    rm -f kfield*.dat ofield*.dat qfield*.dat
    rm -f cs_*.dat snut_*.dat svo_*.dat svo2_*.dat
    rm -f *.info
    ...
    rm -f FSI_position* FSI_Angle* FSI_DATA* DATA_FSI*
...
Examples:
  ...
  # Run full workflow with GPU enabled
  ENABLE_GPU=1 ./run.sh
  ...
  # Quick test run (fewer processes, faster)
  NP_SIM=2 ./run.sh
...
Test Case Description:
  ...
  Expected behavior:
    - Initial transient as vortex street develops
    - Lock-in phenomenon (synchronization of vortex shedding with motion)
    - Large amplitude oscillations in Y-direction
    - Steady-state sinusoidal cylinder displacement

  Physical mechanism:
    Alternating vortex shedding creates fluctuating lift forces on the
    cylinder. When the shedding frequency approaches the natural frequency
    of the spring-mass system (lock-in), the cylinder undergoes large
    amplitude oscillations that synchronize with the vortex shedding.

Output Files:
  Simulation:
    - ufield*.dat, pfield*.dat, etc. (PETSc binary format)
    - FSI_position00 (cylinder displacement history)
    - Force_Coeff_SI00_00 (lift/drag coefficients)
    - surface*.dat (IBM surface mesh evolution)
  ...
  Analysis suggestions:
    - Plot FSI_position00: Y-displacement vs time
    - Compute oscillation amplitude from steady-state displacement
    - Calculate Strouhal number from force coefficient frequency
    - Visualize vortex shedding using Q-criterion in ParaView
...
case "${1:-all}" in
    ...
    all|"")
        ...
        echo "  4. Use 'Calculator' to compute vorticity: curl(Velocity)"
        echo "  5. Animate to see vortex shedding and cylinder motion"
        echo ""
        echo "  6. Analyze cylinder motion:"
        echo "     Plot FSI_position00 (column 2 = Y displacement vs time)"
        echo "     Look for lock-in: steady sinusoidal oscillation"
        echo ""
        ;;
    *)
