...
# Test Case: Cylinder falling under gravity in two-phase flow
#
# Usage:
...
do_simulate() {
    ...
    echo "  Physics:       Falling cylinder in two-phase flow"
    echo "  Method:        Level Set (interface tracking)"
    echo "  Re:            10"
    echo "  Density ratio: 1000:1 (heavy:light fluid)"
    echo "  Mass ratio:    m* = 0.25"
    ...
    # Check for sloshing/levelset output
    if [ -f "sloshing.dat" ]; then
        echo "Level set interface data saved."
    fi
}
...
do_postprocess() {
    ...
    # Note: Level Set case - include levelset output
    POST_CMD="${BUILD_DIR}/Source/data -tis ${TIS} -tie ${TIE} -ts ${TS} -vtk 1 -xyz 1 -binary 1 -levelset 1"
...
do_clean() {
    ...
    print_step "Removing FSI and Level Set output files..."
    rm -f FSI_DATA* fsi_data*.dat
    rm -f sloshing.dat mass.dat shear_velocity.dat
...
Examples:
  ...
  # Post-process all timesteps
  TIS=0 TIE=500 TS=25 ./run.sh postprocess

  # Quick test run
  NP_SIM=2 ./run.sh

Test Case Description:
  This test case simulates a cylinder falling under gravity through a
  two-phase fluid. The Level Set method is used for interface tracking
  between the heavy (lower) and light (upper) fluids.

  Physical parameters:
    - Reynolds number: 10 (based on cylinder diameter and settling velocity)
    - Gravity: -1 m/s^2 (Z-direction)
    - Density ratio: 1000:1 (heavy fluid : light fluid)
    - Mass ratio: m* = 0.25
    - Cylinder initial position: (0, 0, 1.25)

  Expected behavior:
    - Cylinder accelerates downward due to gravity
    - Interface between fluids deforms as cylinder passes through
    - Wake formation behind the falling cylinder
    - Terminal velocity reached when drag balances buoyancy

Output Files:
  Simulation:
    - ufield*.dat, pfield*.dat, etc. (PETSc binary format)
    - nvfield*.dat (Level Set field)
    - FSI_DATA00 (cylinder displacement history)
    - sloshing.dat (interface position)
  ...
  Analysis:
    ...
    - Visualize Level Set field to see interface evolution
    - Compute settling velocity from displacement data
...
case "${1:-all}" in
    ...
    all|"")
        ...
        echo "  3. Apply 'Contour' filter to visualize Level Set interface"
        echo "  4. Apply 'Glyph' filter to visualize velocity vectors"
        echo "  5. Animate to see cylinder falling through interface"
        echo ""
        echo "  6. Plot FSI_DATA00 to analyze cylinder displacement"
        echo "  7. Check sloshing.dat for interface position"
        echo ""
        ;;
    *)
